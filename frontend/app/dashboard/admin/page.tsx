  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow p-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          {dashboard?.users?.byRole && Object.entries(dashboard.users.byRole).map(([role, count]: [string, any]) => (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-lg shadow p-6">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-semibold flex items-center">
              <FontAwesomeIcon icon={faMapLocationDot} className="mr-2 text-orange-600" />
              Sites récents
            </h2>
            <Link
              href="/dashboard/admin/sites"
          ...
          <div className="space-y-3">
            {recentSites.length > 0 ? (
                ...
                <div key={s.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                  <div>
                    <p className="font-medium text-gray-900 flex items-center">
                      <FontAwesomeIcon icon={faLocationDot} className="mr-2 text-gray-400 text-xs" />
                      {s.name}
                    </p>
                    <p className="text-sm text-gray-500 ml-5">{s.client?.name || 'N/A'}</p>
                  </div>
                  <span className={`px-2 py-1 rounded-full text-xs ${
              ...
              <p className="text-gray-500 text-center py-4">Aucun site récent</p>
            )}
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg shadow p-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          >
            <FontAwesomeIcon icon={faUserPlus} className="text-2xl mb-2 text-indigo-600" />
            <span className="font-medium text-indigo-700 block mt-2">Nouvel utilisateur</span>
          </Link>
          <Link
            href="/dashboard/admin/clients/create"
            className="p-4 bg-purple-50 rounded-lg hover:bg-purple-100 transition-colors text-center"
          >
            <FontAwesomeIcon icon={faBuildingUser} className="text-2xl mb-2 text-purple-600" />
            <span className="font-medium text-purple-700 block mt-2">Nouveau client</span>
          </Link>
          <Link
            href="/dashboard/admin/sites/create"
            className="p-4 bg-orange-50 rounded-lg hover:bg-orange-100 transition-colors text-center"
          >
            <FontAwesomeIcon icon={faMapLocationDot} className="text-2xl mb-2 text-orange-600" />
            <span className="font-medium text-orange-700 block mt-2">Nouveau site</span>
          </Link>
          <Link
            href="/dashboard/admin/reports"
            className="p-4 bg-emerald-50 rounded-lg hover:bg-emerald-100 transition-colors text-center"
          >
            <FontAwesomeIcon icon={faChartBar} className="text-2xl mb-2 text-emerald-600" />
            <span className="font-medium text-emerald-700 block mt-2">Générer rapport</span>
          </Link>
        </div>
      </div>
              ...
              <p className="text-lg font-medium flex items-center mt-2">
                <FontAwesomeIcon 
                  ...
                  className={`mr-2 text-xs ${
                    dashboard.system.health.database === 'connected' ? 'text-green-500' : 'text-red-500'
                  }`} 
                />
            ...
            <div className="bg-gray-50 rounded-lg p-4">
              ...
              <p className="text-lg font-medium flex items-center mt-2">
                <FontAwesomeIcon 
                  icon={faCircle} 
                  className={`mr-2 text-xs ${
                    dashboard.system.health.storage === 'ok' ? 'text-green-500' : 'text-yellow-500'
                  }`} 
                />
            ...
            <div className="bg-gray-50 rounded-lg p-4">
              ...
              <p className="text-lg font-medium flex items-center mt-2">
                <FontAwesomeIcon 
                  icon={faCircle} 
                  className={`mr-2 text-xs ${
                    dashboard.system.health.cache === 'ok' ? 'text-green-500' : 'text-yellow-500'
                  }`} 
                />
