...
export default function ControleurReportsPage() {
  ...
  const getPresenceIcon = (value: 1 | 0 | null) => {
    if (value === 1) return <FontAwesomeIcon icon={faCheckCircle} className="text-green-600" />;
    if (value === 0) return <FontAwesomeIcon icon={faTimesCircle} className="text-red-600" />;
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow p-4">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div className="flex space-x-2">
            <button
              ...
              className={`px-4 py-2 rounded-lg text-sm font-medium ${
                period.type === 'day' ? 'bg-indigo-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
              }`}
            >
              Jour
            </button>
            <button
              ...
              className={`px-4 py-2 rounded-lg text-sm font-medium ${
                period.type === 'week' ? 'bg-indigo-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
              }`}
            >
              Semaine
            </button>
            <button
              ...
              className={`px-4 py-2 rounded-lg text-sm font-medium ${
                period.type === 'month' ? 'bg-indigo-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
              }`}
            >
              Mois
            </button>
          ...
          <div className="flex items-center space-x-3">
            ...
            <span className="font-medium text-gray-700">{formatPeriodLabel()}</span>
      ...
      {isLoading ? (
        ...
        <>
            ...
            <>
              ...
              <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                <div className="bg-white rounded-lg shadow p-4">
                  ...
                  <p className="text-2xl font-bold text-indigo-600">{summary.totalSites}</p>
                </div>
                <div className="bg-white rounded-lg shadow p-4">
                  <p className="text-sm text-gray-500">Agents</p>
                  <p className="text-2xl font-bold text-blue-600">{summary.totalAgents}</p>
                </div>
                <div className="bg-white rounded-lg shadow p-4">
                  <p className="text-sm text-gray-500">Présences</p>
                  <p className="text-2xl font-bold text-green-600">{summary.totalPresences}</p>
                </div>
                <div className="bg-white rounded-lg shadow p-4">
                ...
                <div className="bg-white rounded-lg shadow p-4">
                  ...
                  <p className="text-2xl font-bold text-purple-600">{summary.presenceRate.toFixed(1)}%</p>
                </div>
              </div>
                  ...
                  <table className="w-full">
                    ...
                    <tbody>
                      {crossTable?.sites.map((site) => {
                        const siteRows = crossTable.matrix.filter(r => r.siteId === site.id);
                        const totalPresent = siteRows.reduce((sum, r) => sum + r.totalPresent, 0);
                        const totalAbsent = siteRows.reduce((sum, r) => sum + r.totalAbsent, 0);
                        const totalUnknown = siteRows.reduce((sum, r) => sum + r.totalUnknown, 0);
                        const total = totalPresent + totalAbsent + totalUnknown;
                        const rate = total > 0 ? (totalPresent / (totalPresent + totalAbsent)) * 100 : 0;
                        
                        return (
                          <tr key={site.id} className="border-b hover:bg-gray-50">
                            <td className="py-2">{site.name}</td>
                            <td className="text-center">{siteRows.length}</td>
                            <td className="text-center text-green-600">{totalPresent}</td>
                            <td className="text-center text-red-600">{totalAbsent}</td>
                            <td className="text-center text-gray-400">{totalUnknown}</td>
                            <td className="text-center font-medium">{rate.toFixed(0)}%</td>
                          </tr>
                        );
                      })}
                    </tbody>
          ...
          {/* Vue Tableau croisé */}
          {viewMode === 'table' && crossTable && (
            <div className="bg-white rounded-lg shadow overflow-x-auto">
              <div className="p-4 border-b">
                <h2 className="font-semibold">Tableau croisé - Présences par jour</h2>
              ...
              <div className="p-4">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b">
                      <th className="text-left py-2 sticky left-0 bg-white">Site / Agent</th>
                      ...
                      <th className="text-center py-2 bg-gray-50">Total</th>
                    </tr>
                  </thead>
                  <tbody>
                    {crossTable.matrix.map((row, idx) => (
                      <tr key={`${row.agentId}-${row.siteId}`} className="border-b hover:bg-gray-50">
                        <td className="py-2 sticky left-0 bg-white">
                          <div>
                            <span className="font-medium">{row.agentName}</span>
                            <span className="text-gray-400 text-xs ml-2">{row.siteName}</span>
                          </div>
                        </td>
                        {crossTable.dates.map((date) => (
                        ...
                        <td className="text-center py-2 bg-gray-50 font-medium">
                          <span className="text-green-600">{row.totalPresent}</span>
                          <span className="text-gray-400 mx-1">/</span>
                          <span className="text-gray-600">{row.totalPresent + row.totalAbsent + row.totalUnknown}</span>
                        </td>
                      </tr>
                    ))}
                  </tbody>
          ...
          <p className="text-xs text-gray-400 text-right">
            Rapport généré le {new Date(summary.generatedAt).toLocaleString('fr-FR')}
          </p>
        </>
      )}
    </div>
