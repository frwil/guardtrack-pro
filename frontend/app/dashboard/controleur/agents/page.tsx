...
export default function ControleurAgentsPage() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      console.log("📋 Agents récupérés:", agentsList.length);
      ...
      const results = await Promise.all(
        agentsList.map(async (agent) => {
          try {
            const presences = await presencesService.list({
              agentId: agent.id,
              date: today,
            });
            console.log(
              `📍 Agent ${agent.id} (${agent.fullName}): ${presences.length} présence(s)`,
            );
            return { agentId: agent.id, presences };
          } catch (error) {
      ...
      // Remplir le map APRÈS que toutes les promesses soient résolues
      results.forEach(({ agentId, presences }) => {
        if (presences.length > 0) {
          presencesMap[agentId] = presences[0];
        }
      });
  ...
  const filterAgents = () => {
    ...
    if (filter === "present") {
      filtered = filtered.filter((a) => {
        ...
        return (
          presence &&
          (presence.status === "VALIDATED" || presence.status === "PENDING")
        );
      });
          ...
          <p className="text-2xl font-bold text-green-600">
            {
              agents.filter((a) => {
                ...
                return (
                  p && (p.status === "VALIDATED" || p.status === "PENDING")
                );
              }).length
            }
          </p>
                ...
                <div key={agent.id} className="p-4 hover:bg-gray-50">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center space-x-4">
                      <span
                        className={`px-3 py-1 rounded-full text-xs flex items-center ${status.color}`}
                      >
                      ...
                      {presence && (
                        <div className="text-sm text-gray-500">
                          ...
                          {new Date(presence.checkIn).toLocaleTimeString(
                            "fr-FR",
                          )}
                          {presence.site && (
