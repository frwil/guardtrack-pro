      ...
      const results = await Promise.all(
        agentsList.map(async (agent) => {
          try {
            ...
            console.error(`Erreur présences pour agent ${agent.id}:`, error);
            return { agentId: agent.id, presences: [] };
          }
        })
      );
                ...
                <div key={agent.id} className="p-4 hover:bg-gray-50">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center space-x-4">
                      ...
                      <Link href={`/dashboard/controleur/agents/${agent.id}`} className="text-indigo-600 hover:text-indigo-800">
                        Voir détails →
                      </Link>
                    </div>
                  </div>
